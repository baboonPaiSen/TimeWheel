...
public class TimingWheel {
    ...
    /**
     * 时间轮由多个时间格组成，每个时间格就是tickMs，它代表当前时间轮的基本时间跨度
     */
    private Long tickMs;
    /**
     * 代表每一层时间轮的格数
    ...
    /**
     * 时间轮的槽位数组，每个槽位是一个TimerTaskList  每一层的每一个槽中的时间任务列表
     */
    ...
    /**
     * 根据任务的到期时间expiration，决定任务应该放置在哪个时间轮的哪个槽位
     * 如果任务的到期时间小于当前时间加上一个槽位的时间 tickMs，则任务已经过期，返回false
     * 如果任务的到期时间小于当前时间加上时间轮的总时间间隔interval，则任务放置在当前时间轮的相应槽位
     * 如果任务的到期时间大于当前时间轮的最大支持时间，则递归地将任务添加到上层时间轮
